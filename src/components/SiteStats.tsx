...
import { Users, Search, Link2, MousePointerClick, Database } from "lucide-react";
...
export function SiteStats() {
  ...
  const stats = useMemo(() => {
    if (real && ga) {
      return [
        { label: "Ziyaret (Oturum)", value: formatCompact(ga.sessions), sub: `${ga.rangeLabel} · ${ga.sessionsDelta >= 0 ? "+" : ""}%${ga.sessionsDelta}`, icon: Users, color: "#3b82f6" },
        { label: "Organik Arama", value: formatCompact(ga.organicSessions), sub: `Toplamın %${ga.sessions > 0 ? Math.round((ga.organicSessions / ga.sessions) * 100) : 0}'i`, icon: Search, color: "#22c55e" },
        { label: "Kullanıcı", value: formatCompact(ga.users), sub: `${ga.usersDelta >= 0 ? "+" : ""}%${ga.usersDelta} önceki döneme göre`, icon: Link2, color: "#f59e0b" },
        { label: "Hemen Çıkma", value: `%${ga.bounceRate}`, sub: `Etkileşim %${ga.engagementRate} · ${Math.floor(ga.avgDurationSec / 60)}dk ${ga.avgDurationSec % 60}sn`, icon: MousePointerClick, color: "#ef4444" },
      ];
    }
    ...
    return [
      { label: "Toplam Ziyaret", value: formatCompact(isWeekly ? (wd?.visits || 0) : info.visits), sub: isWeekly ? "Haftalık (örnek)" : "Aylık · Haziran 2026 (örnek)", icon: Users, color: "#3b82f6" },
      ...
      <div className="flex items-center gap-1.5 text-[10px] text-zinc-600">
        {real ? (
          <>
            ...
            <span>· {ga?.property} · {ga?.start} → {ga?.end} (Europe/Istanbul)</span>
          </>
        ) : (
